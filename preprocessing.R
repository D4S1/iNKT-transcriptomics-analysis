...
annotations <- lapply(1:4, function(pool_num){
  ...
  pool_name <- paste('pool', pool_num, sep='')
  pool_annotation <- filter(donors_translator_df, pool==pool_name)
  
  annotation <- merge(pool_donors[[pool_num]], pool_annotation, by='pool_donor', all.x=TRUE)
  rownames(annotation) <- rownames(pool_donors[[pool_num]])
  return (annotation[-1])
})
...
create_exp_gene_plot <-function(so_list, filename){
  plot_list <- lapply(so_list, function(so){
    par(mar = c(4, 8, 2, 1))
    ...
    tmp <- data.frame(t(C[most_expressed, ])) # cells in rows, genes in columns
    tmp <- pivot_longer(tmp, cols = colnames(tmp))
    exp_gene_plot <- ggplot(tmp, aes(x = value, y = name, fill = name)) + geom_boxplot(show.legend = FALSE) + ggtitle(so@project.name)
    return (exp_gene_plot)
  })
  
  final_plot <- do.call(grid.arrange, c(plot_list, ncol = 1))
  ggsave(paste('graphics/', filename, '.png', sep='' ), final_plot, width = 7.5, height = 15)
  return (final_plot)
}

highest_exp_genes_plot <- create_exp_gene_plot(so_list, 'hightest_exp_genes')


so_list <- lapply(so_list, function(so){
  tmp <- subset(so, features=setdiff(rownames(so), c('MALAT1')))$RNA
  so$RNA <- tmp
  # we don't omit TMSB4X because is related to leukocyte specialization process
  return (so)
})
